...
<section class="review-layout">
  <div class="stack">
    ...
    <section class="panel"><h2>让 Claude 定向修订</h2><p>该操作固定调用 Azure Anthropic `salesnail-cs-46`，随后由 `gpt-5.5` 独立审阅。</p><label>修订反馈<textarea id="editorial-feedback" rows="4" placeholder="例如：开头更快进入冲突，保留所有有来源的数字。"></textarea></label><button class="button secondary" id="model-editorial">生成新版本</button></section>
  </div>
  <aside class="stack">
...
<section class="review-layout">
  <div class="stack">
    ...
    <section class="panel"><h2>让 Claude 定向修订</h2><p>固定通过 Azure Anthropic Messages API 调用 `salesnail-cs-46`，只修订视觉计划。</p><label>反馈<textarea id="visual-feedback" rows="4" placeholder="说明要修复的场景、信息层级或画面节奏。"></textarea></label><button class="button secondary" id="model-visual">生成修订版本</button></section>
  </div>
