...
背景图采用“共享素材池优先、项目本地取用、缺口再生成”的架构。跨案例复用已经验证过的叙事插画，减少重复生图；每条视频仍保留自己的分镜、素材副本和来源记录。
...
“素材池优先”不等于勉强复用。候选图只有同时支持当前叙事语义和最终构图，才算可用；检索有结果但匹配度不足，仍然属于素材缺口。
...
1. 先把分镜需求写成“空间 + 行为 + 人物 + 叙事职责 + 画风”的检索简报。
2. 至少使用两个维度检索；只搜“会议”通常过宽。
3. 人工检查主体关系、镜头角度、情绪、文字留白和画风。标签相同不代表构图一定合适。
4. 使用 checkout 复制到项目，不手工引用 `assets/visual-pool/files/`。
5. 跨项目取用是默认策略；同一条视频内反复播放同一图片仍需有 callback、对照、证据放大或明确兜底意图，并标记 `reuse` 或 `allowBackgroundReuse`。
6. 没有候选或候选在语义、人物、画风、构图上不够匹配时生成新图。新图通过单图和成片视觉 QA 后回流相应素材池。
